...
module FullcalendarEngine
  class EventsController < ApplicationController
    ...
    def get_events
      ...
      if current_user
        @events = Event.select_user(param_user_or_default("all")).where('
    ...
    def param_user_or_default(default)
      if Event.select_user(params[:user])
        params[:user]
      else
        default
      end
    ...
    def load_event
      if current_user
        @event = Event.select_user(param_user_or_default("all")).where(:id => params[:id]).first
        unless @event
